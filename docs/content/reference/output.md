...
describes what users get. Pick a format with `-o`, or let chucknorris choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
chucknorris <command> -o table   # aligned columns for reading
chucknorris <command> -o jsonl   # one JSON object per line, for piping
chucknorris <command> -o json    # a single JSON array
chucknorris <command> -o csv     # spreadsheet friendly
chucknorris <command> -o tsv     # tab-separated
chucknorris <command> -o url     # just the URL column
chucknorris <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
chucknorris <command> --fields id,title,url
```
...
```bash
chucknorris <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
chucknorris <command>            # a table, because this is a terminal
chucknorris <command> | wc -l    # JSONL, because this is a pipe
```
